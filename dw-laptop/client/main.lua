                        ...
                        for k = 1, searches do
                            if app.bannedJobs[k] == playerJob then
                                if hadApp(app.app) then
                                    TriggerServerEvent('dw-laptop:server:LostAccess', app.app)
                                end
                                goto skip
...
RegisterNetEvent('dw-laptop:client:openlaptop', function()
    if haveItem(Config.LaptopDevice) then
        SetDisplay(true)
    end
end)

RegisterNetEvent('dw-laptop:client:CustomNotification', function(text, icon, color, iconbg, length)
    if haveItem(Config.LaptopDevice) then
        SendNUIMessage({
            action = "custom-notif",
            data = {
...
RegisterNUICallback('laptop/checkout', function(data, cb)
    ...
    QBCore.Functions.TriggerCallback('dw-laptop:server:checkout', function(result)
        if result == "bank" then
            cb({
                status = 'error',
...
RegisterNUICallback('setting/save', function(data, cb)
    ...
    cb("ok")
    TriggerServerEvent("dw-laptop:server:settings:set", data["setting"])
end)
