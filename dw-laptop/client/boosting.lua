...
local function DelayDelivery()
    SetTimeout(math.random(10, 30 * 1000), function()
        TriggerServerEvent('dw-laptop:server:FinalDestination')
    end)
end
        ...
        CreateThread(function()
            ...
            while State and State.boostHacks > 0 do
                ...
                if DoesEntityExist(car) then
                    ...
                    TriggerServerEvent('dw-laptop:server:SyncBlips', pos, NetID)
                else
                    checks = checks + 1
                    if checks >= 3 and not DoesEntityExist(car) then
                        TriggerServerEvent('dw-laptop:server:CancelBoost', NetID, Plate)
            ...
            if DoesEntityExist(car) then
                TriggerServerEvent('dw-laptop:server:SyncBlips', nil, NetID)
        ...
        QBCore.Functions.Progressbar('checking_vin', 'Checking VIN', 10000, false, true,
            { -- Name | Label | Time | useWhileDead | canCancel
                ...
                if not Entity(entity).state.vinchecked then
                    ...
                    QBCore.Functions.TriggerCallback('dw-laptop:server:checkVin', function(cb)
                        res:resolve(cb)
                    end, NetworkID)
    ...
    if DoesEntityExist(car) then
        ...
        if dist <= 3.5 then -- 2.5 is the distance in qbcore vehiclekeys if you use more or less then please edit this.
            if #(vector3(carCoords.x, carCoords.y, carCoords.z) - GetEntityCoords(car)) <= 6.9 then
                AntiSpam = true
                TriggerServerEvent('dw-laptop:server:SpawnPed')
...
RegisterNetEvent('dw-laptop:client:MissionStarted',
...
RegisterNUICallback('boosting/start', function(data, cb)
    QBCore.Functions.TriggerCallback('dw-laptop:server:CanStartBoosting', function(result)
        print(result)
        if result == "success" then
            --TriggerServerEvent('dw-laptop:server:StartBoosting', data.id, currentCops)
            cb({
                status = 'success',
...
local function DeliverCar()
    ...
    TriggerServerEvent('dw-laptop:server:finishBoost', NetID)
...
local function StartVin()
    QBCore.Functions.Progressbar('vin_scratching', 'Scratching VIN', 7000, false, true,
        { -- Name | Label | Time | useWhileDead | canCancel
            ...
            TriggerServerEvent("dw-laptop:server:fuckvin", NetID, model, mods)
            TriggerServerEvent('dw-laptop:server:finishBoost', NetID, true)
...
RegisterNetEvent('dw-laptop:client:ReturnCar', function(coords)
...
RegisterNetEvent("dw-laptop:client:ToVinScratch", function(coords)
    PZone = CircleZone:Create(coords, 15, {
        name = "VinGoesBrum",
        debugPoly = Config.Boosting.Debug
    })
...
RegisterNetEvent('dw-laptop:client:finishContract', function(table)
    if PZone then PZone:destroy() PZone = nil end
...
RegisterNetEvent('dw-laptop:client:HackCar', function()
    ...
    if haveItem(Config.Boosting.HackingDevice) then
        if IsPedInAnyVehicle(ped, false) then
            ...
            if State and State.boostHacks > 0 and not State.boostCooldown then
                ...
                TriggerServerEvent('dw-laptop:server:SyncPlates', success)
...
if Config.Boosting.Debug then
    RegisterCommand('hackcar', function()
        ...
        TriggerEvent('dw-laptop:client:HackCar')
    end, false)
end
...
RegisterNetEvent('dw-laptop:client:SpawnPeds', function(netIds, Location)
    for i = 1, #netIds do
        -- Get the Ped --
...
RegisterNetEvent('dw-laptop:client:SyncBlips', function(coords, newNet)
...
RegisterNetEvent('dw-laptop:client:ContractHandler', function(table, currentdate)
...
RegisterNetEvent('QBCore:Client:OnPlayerLoaded', function()
    QBCore.Functions.TriggerCallback('dw-laptop:server:GetContracts', function(result)
        Contracts = result
...
RegisterNUICallback('boosting/deny', function(data, cb)
    ...
    QBCore.Functions.TriggerCallback('dw-laptop:server:DeclineContract', function(result)
        if result == "success" then
            cb({
                status = 'success',
...
RegisterNUICallback('boosting/transfer', function(data, cb)
    ...
    local contract = data.contractID
    QBCore.Functions.TriggerCallback('dw-laptop:server:TransferContracts', function(result)
        if result == "success" then
            cb({
                status = 'success',
...
RegisterNUICallback("boosting/cancel", function(data, cb)
    TriggerServerEvent('dw-laptop:server:CancelBoost', NetID, nil, true)
    cb("success")
end)
...
RegisterNUICallback('boosting/queue', function(data, cb)
    QBCore.Functions.TriggerCallback('dw-laptop:server:joinQueue', function(result)
        if result == "success" then
            if data.status then
                cb({
                    status = 'success',
...
RegisterNetEvent('dw-laptop:client:QueueHandler', function(value)
    inQueue = value
end)


RegisterNetEvent('dw-laptop:client:setvehicleFuel', function(veh)
