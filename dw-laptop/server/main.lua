...
QBCore.Functions.CreateUseableItem(Config.LaptopDevice, function(source, item)
    ...
    if Player.Functions.GetItemByName(Config.LaptopDevice) then
        TriggerClientEvent('dw-laptop:client:openlaptop', source)
    end
end)
...
RegisterNetEvent('dw-laptop:server:LostAccess', function(app)
    local src = source
    if app == "boosting" then
        TriggerEvent("dw-laptop:server:QuitQueue", src)
    end
end)


RegisterNetEvent('dw-laptop:server:settings:set', function(setting)
    local src = source
...
RegisterNetEvent('dw-laptop:server:RemoveItem', function(item)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
